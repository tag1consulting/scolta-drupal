...
/**
 * `drush scolta:reindex` refreshes indexed output without an entity save.
 *
...
class ReindexCommandKernelTest extends KernelTestBase {
  ...
  /**
   * Only published, indexable IDs are queued, and never the entity itself.
   */
  public function testIdsScopeAndUnknownTypeAreValidated(): void {
    $queue = \Drupal::queue('scolta_rebuild');
    $this->commands()->reindex('node', '', ['ids' => (string) $this->nids[0]]);
