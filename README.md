...
| `{type: <update hook>}` | `scolta_queue_full_rebuild()` from an update hook; with `$force` it also deletes the timestamp manifest | Untargeted; full build, regathering every entity when forced |
