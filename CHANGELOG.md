...
### Fixed
- `drush scolta:download-pagefind` no longer declares a `--version` option. It collided with Drush's global `--version`, so the command failed on startup with "An option named "version" already exists." The command downloads the latest Pagefind release; `--dest` is unchanged.
