...
### Fixed
- **Deleting or unpublishing content now removes its exported HTML on the search_api / Pagefind-binary path.** `ContentExporter::writeManifest()` had no call site in the module, so `PagefindExporter::deleteItem()` read an empty manifest, fell through to a flat `{id}.html` name the nested URL-mirrored layout has not produced since 1.1.0, and deleted nothing: `ScoltaBackend::deleteItems()` was a silent no-op and the page stayed searchable. The backend now writes the manifest after each index and delete batch, merging into what is on disk — a whole-site ID → path map cannot be rebuilt from the one batch Search API hands over, which is why `ContentExporter::writeManifest()`, whose contract is to replace, is not used there. It is called directly from the two paths that do export a whole directory (`drush scolta:export` and the settings form's binary rebuild), both of which empty the directory first. `deleteItem()` now reports whether it removed a file, and a delete that finds nothing is logged rather than passing quietly.
