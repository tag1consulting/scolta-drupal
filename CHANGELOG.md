...
### Fixed
- **Amazee credentials provisioned without resolved model names now self-heal instead of breaking AI permanently.** The Amazee.ai free-trial provisioner persists credentials (LiteLLM token + URL) and the resolved model names in two steps; when the `/model/info` resolution step fails (gateway hiccup, transient network), credentials are stored with **no resolved model**. `AutoProvisioner::ensureAiAvailable()` then no-opped forever (it only checked that credentials were *stored*), `createClient()` fell back to the shipped dated default `claude-sonnet-4-5-20250929`, and the Amazee gateway rejects that dated name with **HTTP 400** — so summarize silently returned nothing and expand ran unexpanded, with no path back. This is a distinct failure class from the expired-key recovery above (a `400 invalid-model` from half-provisioned credentials, not an auth failure). `ScoltaAiService::createClient()` now passes scolta-php's new `hasResolvedModels` predicate to `ensureAiAvailable()`: when credentials are stored but the persisted `ai_model` is still the shipped dated default (or empty), the library re-resolves models against the **already-stored key** (never a fresh trial, which would waste a server-limited allocation) and persists the result — self-healing on the next request. The predicate (`modelIsResolved()`) deliberately treats the dated default as *unresolved*, because config seeds `ai_model` to it out of the box: a naive "is `ai_model` non-empty" check would always report resolved and the self-heal would never fire. As the second half, when credentials are stored but no model is resolved, `createClient()` now degrades to a key-less client (`ApiKeyMissingException` → unexpanded/no-summary HTTP 200) rather than sending the gateway the dated default (HTTP 400) — mirroring `scolta-node`'s `AmazeeAiService::buildClient()`. The global/explicit-key default is unchanged (a dated model is valid on the real Anthropic API); only the Amazee branch degrades. **Requires scolta-php ≥ 1.0.4 (unreleased)** (`AutoProvisioner::ensureAiAvailable()`'s `$hasResolvedModels` parameter); a `composer update tag1/scolta-php` lock bump must land once scolta-php 1.0.4 is tagged, before any scolta-drupal release. Covered by `ModelResolutionSelfHealTest` (predicate reports unresolved for the dated default; the real `AutoProvisioner` re-resolves against the stored key driven by that exact predicate; a naive non-empty predicate provably would *not* heal; the key-less degrade throws `ApiKeyMissingException` and never calls the gateway).
- **Expired Amazee trial keys now recover instead of silently killing AI, and `/health` tells the truth.** An Amazee.ai trial key is revoked server-side when the trial ends; the next AI call then returned `400 expired_key` while `AutoProvisioner::ensureAiAvailable()` no-opped on the stored (dead) credentials — so expand echoed the query, summarize returned nothing, and `/health` still reported `ai_configured: true` (the django demo outage, 2026-06-09). `ScoltaAiService` now wires scolta-php's `KeyExpiryRecovery` into the AI call path: an auth-class failure triggers a one-shot re-provision of a fresh trial key (guarded to one attempt per window) and a single retry, with `createRecoveredClient()` overridden to route that retry through Drupal's HTTP client. Recovery is wired **only** on the auto-provisioned Amazee path — an explicit `SCOLTA_API_KEY`/`settings.php` key or the `drupal_ai` provider never triggers a behind-the-scenes re-provision (a failing explicit key is the user's to fix; budget-exhaustion is excluded so it can't reset the spend ceiling). `HealthController` hands the same `cache.default` backend to `HealthChecker`, so a recorded auth failure surfaces as `ai_usable: false` / `ai_auth_failing: true` (status `degraded`) while the key is known-bad. **Requires scolta-php ≥ 1.0.4 (unreleased)** (`AiServiceAdapter::setKeyExpiryRecovery()`, `KeyExpiryRecovery`, `HealthChecker`'s `$cache` parameter); a `composer update tag1/scolta-php` lock bump must land once scolta-php 1.0.4 is tagged, before any scolta-drupal release. Covered by `KeyExpiryRecoveryWiringTest` (behavioral recover-and-retry on the Amazee path, no re-provision over an explicit key, health truthfulness through the Drupal cache bridge).
...
### Fixed
- **Baselined runtime fatal in the cron rebuild worker.** `ScoltaRebuildWorker`'s fingerprint-write error path called `$this->getLogger()`, which `QueueWorkerBase` does not have — the error path itself fataled, and the "Call to an undefined method" PHPStan error had been baselined instead of fixed. The worker now implements `ContainerFactoryPluginInterface` with all dependencies injected (lock, config, file system, stream wrappers, entity type manager, state, cache-tags invalidator, logger), the fingerprint write is extracted to `writeFingerprint()` which logs and never throws, and the baseline entry is gone. Unit tests exercise the failure and success paths.
...
- **AI controller triplication extracted into `AiApiControllerBase`.** `ExpandQueryController` / `SummarizeController` / `FollowUpController` were ~95% identical (constructor, `create()`, cache/generation/enricher resolution, JSON decode, error shape). The base now owns the whole request flow — flood check → `parseJsonBody()` (the new scolta-php shared decoder; controllers no longer hand-roll `json_decode` + 400 blocks) → one abstract `invokeHandler()` → shared success/error/limit response mapping. Each endpoint controller is now just its `invokeHandler()`; `FollowUpController` keeps its never-cache semantics by overriding `resolveCache()`. **Uses scolta-php ≥ 1.0.4 (unreleased) `AiControllerTrait::parseJsonBody()`** — same release-gating note as the `isBudgetError()` adoption: lock bump required after scolta-php 1.0.4 is tagged, before any scolta-drupal release.
