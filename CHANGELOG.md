...
### Changed
- **Breaking: Drupal cron no longer runs the `scolta_rebuild` queue.** The worker's `cron = {"time" = 120}` annotation is gone; rebuilds run from an external cron line, every minute, in every environment that should index: `* * * * * drush queue:run scolta_rebuild`. Web cron bounded the build by the request's execution time, and a site that defined no `drush cron` job at all — Share My Lesson staging — never drained the queue, while the cli pod running `drush scolta:build` by hand was killed twice in one day. **Upgrade step:** add the cron line (on Lagoon, an in-pod `cli` cronjob with schedule `M * * * *` running `mail_log.sh "drush queue:run scolta_rebuild"`); nothing else changes. Sites without a drush cron keep the settings form's *Index now*. The worker enqueues its `{op: resume}` marker before the first segment of a full build runs, not after a yield, and deletes it only when the build completes or is given up on, so a process killed mid-segment leaves a claimable request whatever `--lease-time` was and the next tick continues from the last committed page; duplicates are drained to one, and a marker with no interrupted build behind it asks for a full build. A yielded segment is now chained to completion within the same tick through scolta-php's `ResumeChainRunner`, spawning `drush scolta:build --resume` children and renewing the build lock while they run; where drush cannot be spawned the worker runs one segment per tick as before. The standing guidance to run `drush scolta:build` as part of deploys is withdrawn: a release that changes the index format queues its own rebuild and says so.
- `ScoltaCommands::runResumeChain()` drives its segments through `Tag1\Scolta\Index\ResumeChainRunner` (scolta-php ≥ 1.5.0). `ScoltaCommands::RESUME_SEGMENT_ENV` is gone; the environment contract is `ResumeChainRunner::SEGMENT_ENV`. The command and the queue worker each held a copy of entity-type resolution, memory-budget resolution, directory resolution, the streamed segment and the chain; those now live in one service, `Drupal\scolta\Service\IndexBuildRunner` (`scolta.index_build_runner`), that both call. `ScoltaCommands` and `ScoltaRebuildWorker` take it as a constructor argument.

### Added
- `drush scolta:request-build` (`srb`): enqueues one full-rebuild request for the next `queue:run scolta_rebuild` tick, adding nothing when a request is already waiting. For operators and deploy scripts; `scolta_reset_index_state()` keeps enqueueing for update hooks as before.
...
### Fixed
- **`scolta_reset_index_state()` no longer discards build state that is already in the format it resets to.** The function (called by `scolta_update_10007()`) decided from the database's schema version, while the state it empties lives on the files mount; when the two diverged — a database rollback, an environment that copies the production database in on every deploy (Lagoon `SYNC_ON_DEPLOY`), a re-run after a failed deploy — the hook ran again and threw away an in-progress build (a 2.5-hour build on Share My Lesson staging, twice). The reset now writes a `state-format` marker file into the build state directory, and every build path (`drush scolta:build`, the queue worker, the settings form's *Index now*) writes it when it prepares the directory; a reset whose marker is already at or past the target format returns without emptying anything or queueing a rebuild. A stale `segment-outcome.json` from a failed resume plays no part in the decision.
- **A hand-launched `drush scolta:build --resume` now drives resume segments to completion, as a fresh build does.** The command took every `--resume` for a segment spawned by its own chain, which must yield to its parent rather than chain on itself, so an operator resuming an interrupted build got one segment per command and the message "Re-run `drush scolta:build --resume` to continue" each time (Share My Lesson staging, 124,053 entities: 55,050, then 104,000, then 112,300 pages, one command each). Segments spawned by `runResumeChain()` now carry `SCOLTA_RESUME_SEGMENT=1` in their environment, and only a process that sees it reports to a parent instead of chaining. Covered by `ResumeChainKernelTest`.
