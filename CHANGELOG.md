...
### Added
- **`scolta_queue_full_rebuild($reason, $force)` in scolta.install queues a full rebuild from a consuming site's update hook.** With `$force` it first deletes the timestamp manifest so the build regathers every entity, the case after a change to `hook_scolta_content_item_alter()`; without it a queued full rebuild is mostly skipped while the manifest is current. The mild counterpart to `scolta_reset_index_state()`, which empties the whole state directory and only acts across format bumps.
