...
### Removed
- **The PHP indexer is the only pipeline: the Search API backend and the Pagefind binary pipeline are gone.** ([#280](https://github.com/tag1consulting/scolta-drupal/issues/280)) Two systems defined the index and could disagree: Search API's datasources and tracker fed `ScoltaBackend`, which rendered items to HTML that the PHP path never read and enqueued a bare rebuild marker that forced a full build on every indexing run, while the gatherer walked entity storage itself and owned the page IDs, the ledger and the incremental updater. `scolta.settings: entity_types` is now the single definition of what is indexed. Removed: `ScoltaBackend`, `PagefindExporter`, `PagefindBuilder` and their services; `drush scolta:export`, `scolta:rebuild-index` and `scolta:download-pagefind`; the `--indexer`, `--output-dir` and `--skip-pagefind` options of `scolta:build`; the indexer selector on the settings form; the `indexer`, `pagefind.binary` and `pagefind.view_mode` config keys; the `search_api_index_fields` form alter, the binary requirements check, and the `search_api` dependency. `scolta:status` no longer reports `search_api` or `indexer` sections. `scolta_update_10008()` deletes the Search API index and server that used the `scolta_pagefind` backend, carries the server's rebuild delay into the new `pagefind.auto_rebuild_delay` key, and uninstalls `search_api` when nothing else uses it. See UPGRADE.md.
