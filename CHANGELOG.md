...
### Changed
- The DDEV CI job (`functional`) became a two-cell matrix, `integration-lowest` (PHP 8.3, Drupal `~10.3.0`, `ddev poser --prefer-lowest --prefer-stable`) and `integration-highest` (PHP 8.4, newest core and dependencies), each running the kernel and functional suites — the first CI coverage of the claimed Drupal 10.3 floor. Each cell logs its resolved versions.
