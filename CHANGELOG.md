...
- Deleted `AiPermissionBackfillFunctionalTest`: `integration-lowest` reproduced its one real failure locally and traced it to a Drupal core caching quirk (`AccessPolicyProcessor`'s static-cache layer isn't busted by a role save within the same PHP process), not a scolta defect — the test's own pattern of granting a permission and immediately checking `AccountInterface::hasPermission()` in the same process is what triggers it; no real request sequence hits this, since every request gets a fresh container. The other three tests exercising update hooks (`AmazeeModelMigrationTest`, `ManagedGatewayOptInFunctionalTest`, `SaytSettingsFunctionalTest`) check config values or `RoleInterface::hasPermission()` on the reloaded entity, not the account-level access-policy path, so they don't hit it.
