...
### Added
- **`Drupal\scolta\Service\ScoltaReindexer` is now a service alias**, so a consuming site's autowired Drush command or service can type-hint the class and let the container resolve it; before this, `Cannot autowire service "Drupal\scolta\Service\ScoltaReindexer"` unless `@scolta.reindexer` was wired explicitly.
