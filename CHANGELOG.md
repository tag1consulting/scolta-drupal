...
- `AiPermissionBackfillKernelTest`: restores three of the four tests deleted earlier from `AiPermissionBackfillFunctionalTest` (the fourth hit a Drupal core caching quirk — an account-level `hasPermission()` check right after a mid-request role grant can see stale data from `AccessPolicyProcessor`'s static cache, which no production request sequence does). These three assert on the reloaded `Role` entity instead, which doesn't touch that path: the fresh-install baseline, the update backfilling both roles, and idempotency. Anonymous access to the AI endpoints — the one thing that grant actually gates — is otherwise untested; a functional test for that specific case would need to be written fresh, not restored, since the one that existed for it was the broken one.
